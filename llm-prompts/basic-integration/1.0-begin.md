...
Detect the project type by examining which configuration files exist at the project root.

- JavaScript projects have a `package.json file`
- Python projects have `requirements.txt`, `pyproject.toml`, `Pipfile`, or `setup.py`.

Only read the config file that matches the detected project type.

Check if PostHog is already installed:

- JavaScript: `posthog-js` in dependencies
- Python: `posthog` in dependencies

Before proceeding, find any existing PostHog capture code. The method name varies by language:

- JavaScript: `posthog.capture()`
- Python: `posthog.capture()` or `capture()` (context API)

Make note of event name formatting.
